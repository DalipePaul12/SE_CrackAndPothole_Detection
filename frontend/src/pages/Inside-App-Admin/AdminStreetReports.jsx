...
function AdminStreetReports() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    let geoCache = {};
    try {
      geoCache = JSON.parse(sessionStorage.getItem(GEO_CACHE_KEY) || "{}");
    } catch (e) {
      geoCache = {};
    }
  ...
  const rowData = Object.entries(locationGroups).map(([key, data]) => ({
    ...
    potholes: data.reports.filter(
      (r) => damageType(r).toLowerCase() === "pothole"
    ).length,
    cracks: data.reports.filter(
      (r) => damageType(r).toLowerCase() === "crack"
    ).length,
    critical: data.reports.filter(
      (r) =>
        severity(r).toLowerCase().includes("critical") &&
        !severity(r).toLowerCase().includes("non")
    ).length,
  ...
  const filteredRows = rowData.filter((row) => {
    ...
    const matchSearch =
      ...
      row.reports.some((r) =>
        (r.exact_address || "").toLowerCase().includes(q)
      );
    ...
    <div className="asr-container">
      <div className="asr-filters-card">
        <div className="asr-header">
          ...
          <span className="asr-total-badge">
            {allReports.length} Dispatches
          </span>
        </div>
            ...
            {loading ? (
              ...
              filteredRows.map((row) => (
                <React.Fragment key={row.key}>
                  <tr
                    className={`asr-street-row ${
                      expandedRow === row.key ? "expanded" : ""
                    }`}
                    onClick={() =>
                      setExpandedRow((p) => (p === row.key ? null : row.key))
                    }
                  >
                    ...
                    <td>
                      <div className="asr-type-badges">
                        {row.potholes > 0 && (
                          <span className="type-badge pothole">
                            {row.potholes} Pothole
                            {row.potholes > 1 ? "s" : ""}
                          </span>
                        )}
                        {row.cracks > 0 && (
                          <span className="type-badge crack">
                            {row.cracks} Crack{row.cracks > 1 ? "s" : ""}
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td>
                      {row.latestDate
                        ? new Date(row.latestDate).toLocaleDateString()
                        : "—"}
                    </td>
                  </tr>
                      ...
                      <tr className="asr-child-header-row">
                        ...
                        <td colSpan={6}>
                          <div className="asr-child-header">
                            <span>
                              Exact damage coordinates for{" "}
                              <strong>
                                {row.street}, {row.barangay}
                              </strong>
                            </span>
                          </div>
                        </td>
                      ...
                      {row.reports.map((r) => {
                        ...
                        return (
                          ...
                          >
                            <td></td>
                            <td>
                              <strong className="asr-report-id">
                                #{String(r.id).padStart(4, "0")}
                              </strong>
                              <div
                                className="asr-exact-address"
                            ...
                            <td>
                              <span
                                className={`type-badge ${damageType(
                                  r
                                ).toLowerCase()}`}
                              >
                                {damageType(r)}
                            ...
                            <td>
                              <span
                                className={`admin-severity ${severity(r)
                                  .toLowerCase()
                                  .replace(/[\s_]/g, "-")}`}
                              >
                                {severity(r)}
                            ...
                            <td>
                              <span
                                className={`admin-status ${r.status
                                  ?.toLowerCase()
                                  .replace(/[\s_]/g, "-")}`}
                              >
              ...
              <tr>
                <td colSpan="7" className="admin-no-data">
                  No locations match your parameters.
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div className="modal-body">
          <div className="modal-left">
            <div className="reporter-info">
              ...
              <div className="info-row">
                <strong>Contact:</strong> {r.owner?.phone ?? "—"}
              </div>
            </div>
            ...
            <div className="location-info">
              <p>
                <strong>Exact Address:</strong> {displayAddress}
              </p>
              <p>
