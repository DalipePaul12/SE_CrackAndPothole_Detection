...
function resolveTypeKeyFromNotif(notif) {
  ...
  if (baseKey !== "info" && baseKey !== "system") {
    return baseKey;
  }

  const haystack =
    `${notif?.title ?? ""} ${notif?.message ?? ""}`.toLowerCase();
  ...
  if (haystack.includes("deleted")) return "deleted";
  if (haystack.includes("removed")) return "deleted";
...
function NotifCard({
  ...
  return (
    <div
      className={`notif-card ${cfg.cls} ${
        notif.is_read ? "is-read" : "is-unread"
      }`}
      onClick={() => {
        onView(notif);
    ...
    >
      ...
      <div className="notif-card-body">
        ...
        <p className="notif-card-message">
          {notif.message}
        </p>
...
function EmptyState({ activeFilter }) {
  return (
    <div className="notif-empty-state">
      ...
      <p>
        When your reports get updates, they will appear here.
      </p>
    </div>
  );
}
...
export default function Notifications() {
  ...
  const filteredNotifications = useMemo(() => {
    return notifications.filter((notif) => {
      ...
      return (
        resolveTypeKeyFromNotif(notif) === activeFilter
      );
    });
    ...
    return FILTER_TABS.reduce((acc, tab) => {
      if (tab.key === "unread") {
        ...
        acc[tab.key] = notifications.filter(
          (n) =>
            !n.is_read &&
            resolveTypeKeyFromNotif(n) === tab.key
        ).length;
      }
  ...
  const handleView = useCallback(async (notif) => {
    ...
    if (notif.report_id && resolveTypeKeyFromNotif(notif) !== "deleted") {
...
{(reportModal || reportLoading) && (
  <div className="notif-modal-overlay" onClick={() => setReportModal(null)}>
    <div className="notif-modal" onClick={e => e.stopPropagation()}>
      <button className="notif-modal-close" onClick={() => setReportModal(null)}>
        <X size={20}/>
      </button>
      ...
      {reportLoading ? (
        ...
        <>
          <div className="notif-modal-header">
            <h3>Report #{String(reportModal.id).padStart(3,"0")}</h3>
            <span className={`notif-modal-status st-${reportModal.status?.toLowerCase()}`}>
              {reportModal.status}
            </span>
          </div>
          ...
          <div className="notif-modal-timeline">
            {["PENDING","VERIFIED","IN_PROGRESS","RESOLVED"].map((s, i) => {
              const statuses = ["PENDING","VERIFIED","IN_PROGRESS","RESOLVED"];
              const currentIdx = statuses.indexOf(reportModal.status?.toUpperCase());
              ...
              return (
                <React.Fragment key={s}>
                  ...
                  {i < 3 && <div className={`notif-tl-line ${done && i < currentIdx ? "done" : ""}`}/>}
                </React.Fragment>
              );
            })}
          </div>
          ...
          <div className="notif-modal-body">
            <div className="notif-modal-row">
              ...
              <span>{reportModal.barangay ?? "—"}</span>
            </div>
            <div className="notif-modal-row">
              <span className="notif-modal-label">Damage Type</span>
              <span style={{textTransform:"capitalize"}}>{reportModal.ai_damage_type ?? reportModal.damage_type ?? "—"}</span>
            </div>
            <div className="notif-modal-row">
              <span className="notif-modal-label">Severity</span>
              <span style={{textTransform:"capitalize"}}>{reportModal.ai_severity ?? reportModal.severity ?? "—"}</span>
            </div>
            <div className="notif-modal-row">
              <span className="notif-modal-label">Submitted</span>
              <span>{reportModal.created_at ? new Date(reportModal.created_at).toLocaleDateString() : "—"}</span>
            </div>
            {reportModal.decline_reason && (
              <div className="notif-modal-row">
                <span className="notif-modal-label">Reason</span>
                <span style={{color:"#e74c3c"}}>{reportModal.decline_reason}</span>
              </div>
            )}
          </div>

          {/* Photo */}
          {reportModal.media_attachments?.[0]?.file_url && (
            <div className="notif-modal-photo">
              <img
                src={`${import.meta.env.VITE_API_URL || ""}${reportModal.media_attachments[0].file_url}`}
                alt="Report"
