...
def _build_async_url(url: str) -> str:
    ...
    Keeps Supabase pooler URLs intact (just swaps the driver prefix)
    so asyncpg connects via the reachable IPv4 pooler, not the IPv6-only
    direct host.
    """
    url = url.replace("postgresql+psycopg2://", "postgresql+asyncpg://")
    url = url.replace("postgresql://", "postgresql+asyncpg://")
    url = url.replace("postgres://", "postgresql+asyncpg://")
    # Strip sslmode — asyncpg handles SSL natively via its own ssl param
